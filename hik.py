...
class HKCam(object):
    def __init__(self, camIP, username, password, devport=8000):
        ...
        if self.lUserId < 0:  # 登录失败
            ...
            self.init_info = 'Login device fail, error code is: %d' % self.Objdll.NET_DVR_GetLastError()
            print(self.init_info)
            # 释放资源
            self.Objdll.NET_DVR_Cleanup()
            exit()
        else:
            self.init_info = f'摄像头[{camIP}]登录成功!!'
            print(self.init_info)
